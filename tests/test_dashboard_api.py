...
def test_api_async_scan_list_baseline_html(tmp_path: Path, monkeypatch):
    ...
    report_md = client.get(f"/api/scans/{scan_id}/report.md")
    assert report_md.status_code == 200
    assert report_md.text.startswith("# Magic Security audit:")
    assert "Remediation:" in report_md.text
    assert "**Evidence**" in report_md.text
    assert "- Check:" in report_md.text
    ...
    assert "- Workspace:" in report_md.text
    assert report_md.headers["content-type"].startswith("text/markdown")
    assert report_md.headers["cache-control"] == "no-store"
