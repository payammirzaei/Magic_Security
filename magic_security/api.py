...
def create_app(db_path: str | Path = ".magic-security/magic.db"):
    ...
    def get_report_markdown(scan_id: str, workspace_id: str = Query(default="default")) -> PlainTextResponse:
        ...
        lines = [f"# Magic Security audit: {row['target_id']}", "", f"- Scan: `{scan_id}`", f"- Created: `{row.get('created_at', 'unknown')}`", f"- Findings: **{summary.get('findings', 0)}**", f"- Severity: **{severity_line}**", f"- Validity: **{validity.get('status', 'unknown')}**", f"- Checks executed: **{validity.get('checks_executed', 0)}**", "", "## Findings"]
        for finding in report.get("findings", []):
