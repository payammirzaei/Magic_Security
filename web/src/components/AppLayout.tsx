...
export function AppLayout() {
  ...
  useEffect(() => { const timer = window.setInterval(() => { api.refreshSession().then((session) => window.localStorage.setItem('magic_security_api_key', session.token)).catch(() => undefined) }, 15 * 60 * 1000); return () => window.clearInterval(timer) }, [])
  const logout = async () => { try { await api.deleteSession() } finally { clearApiToken(); window.location.assign('/login') } }
