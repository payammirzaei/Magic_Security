...
export function LoginPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(null); try { const session = await api.createSession(token.trim()); setApiToken(session.token); await api.me(); navigate('/', { replace: true }) } catch { setError('Token is invalid or the API is unavailable.'); localStorage.removeItem('magic_security_api_key') } finally { setBusy(false) } }
